...
export function ProjectBrowser() {
  ...
  const loadMetadata = async () => {
    try {
      const response = await fetch(`${BASE_PATH}/data/gemeentelijke-investeringen/projects_metadata.json`)
  ...
  const loadChunk = async (chunkIndex: number) => {
    ...
    try {
      const response = await fetch(
        `${BASE_PATH}/data/gemeentelijke-investeringen/projects_2026_chunk_${chunkIndex}.json`
      )
      ...
      setError(`Kon data chunk ${chunkIndex} niet laden`)
      setLoading(false)
    }
  }
